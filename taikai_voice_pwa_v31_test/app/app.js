...
function add(value,recognized=true,rawSpeech="",captureTs=0,confidence=0,inputType=""){
  ...
  flashAccepted(value,confidence,!recognized||value==="ムリ");records.unshift(rec);if(cfg.endpoint)sendQueue.push(rec);save();render();(window.v31ProcessQueue?window.v31ProcessQueue():processQueue());if(suspiciousRepeat)$("status").textContent=`⚠ ${value}：${duplicateSeconds}秒前にも受付（重複候補）`;
