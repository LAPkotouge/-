...
function makeRecordBase(value,recognized,rawSpeech,captureTs,confidence=0,inputType="voice"){
  const recognizedTs=Date.now(), acceptedTs=Number(captureTs)||recognizedTs;
  return {id:recognizedTs+"_"+Math.random().toString(36).substr(2,5),seqNo:nextSeq(),value,recognized,time:timeFromTs(acceptedTs),ts:acceptedTs,captureTs:acceptedTs,recognizedTs,recognitionDelayMs:Math.max(0,recognizedTs-acceptedTs),confidence:Number(confidence)||0,inputType:String(inputType||"voice"),event:cfg.event,date:cfg.date,point:cfg.point,staff:cfg.staff,mode:cfg.mode,rawSpeech,sheetId:cfg.sheetId||""};
}

function add(value,recognized=true,rawSpeech="",captureTs=0,confidence=0,inputType="voice"){
  if(rawSpeech)lastSpeech=rawSpeech;
  if(cfg.mode==="RELAY"&&recognized){const prev=relayLast(value);if(prev){const diff=(Date.now()-prev.ts)/1000;if(diff<cfg.relayGap*60){const rec={...makeRecordBase(value,true,rawSpeech,captureTs,confidence,inputType),invalidGap:true,invalidSeconds:Math.round(diff),lap:prev.lap};records.unshift(rec);save();render();$("status").textContent=`⚠ ${value}：${cfg.relayGap}分未満なので無効`;return;}}const lap=prev?(Number(prev.lap)||1)+1:1;const rec={...makeRecordBase(value,true,rawSpeech,captureTs,confidence,inputType),duplicate:false,lap};records.unshift(rec);if(cfg.endpoint)sendQueue.push(rec);save();render();processQueue();return;}
...
function registerManual(){const v=parseNumber($("numberInput").value.trim());if(v==="CANCEL"){cancelLast();$("numberInput").value="";return}if(!v){alert(cfg.mode==="RELAY"?"1～99999の番号を入力してください。":manMode?"万台モードでは10,000～99,999を入力するか1桁ずつ読んでください。":cfg.mode==="EKIDEN"?"駅伝は 125-3（1～9999×1～25区）で入力してください。":"1～99999の番号を入力してください。");return}add(v==="MURI"?"ムリ":v,v!=="MURI",$("numberInput").value.trim(),0,0,v==="MURI"?"muri":"manual");$("numberInput").value="";$("numberInput").focus();}
...
function fillGapWithMuri(){
  ...
  for(let i=0;i<n;i++)add("ムリ",false,"集団人数抜け補完",base+i*250,0,"muri");
...
function addPack(values,rawSpeech,captureTs,confidence=0){
  ...
  unique.forEach((v,i)=>add(v,true,rawSpeech,base+i*250,confidence,"voice"));
...
if(packMode){const pack=parsePackNumbers(t);if(pack.length>=2){addPack(pack,t,captureTs,confidence);return;}if(packExpectedCount>=2&&pack.length===1){add(pack[0],true,t,captureTs,confidence,"voice");warnPackCountMismatch(packExpectedCount,1,pack,captureTs);packExpectedCount=0;renderPackCountControls();return;}}
const v=parseNumber(t);if(v==="CANCEL")cancelLast();else if(v==="MURI")add("ムリ",false,t,captureTs,confidence,"muri");else if(v)add(v,true,t,captureTs,confidence,"voice");else $("status").textContent=`認識できません：「${t}」`;};recognition.onerror=e=>{speechCandidateTs=0;if(e.error!=="aborted")$("status").textContent="音声認識エラー："+e.error;};recognition.onend=()=>{if(listening){clearTimeout(restartTimer);restartTimer=setTimeout(()=>{try{recognition.start()}catch{}},180)}};listening=true;render();try{recognition.start()}catch{}}
