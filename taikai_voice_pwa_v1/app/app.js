...
function topRemain(){
  if(!cfg.top)return "未設定";
  const p=cfg.top.split(":").map(Number); if(p.length<2)return "未設定";
  const n=new Date(),t=new Date(); t.setHours(p[0],p[1],p[2]||0,0);
  let sec=Math.floor((t-n)/1000), sign=sec<0?"+":"-"; sec=Math.abs(sec);
  return sign+String(Math.floor(sec/3600)).padStart(2,"0")+":"+String(Math.floor(sec%3600/60)).padStart(2,"0")+":"+String(sec%60).padStart(2,"0");
}

function renderHistory(){
  const h=$("history"); if(!h)return;
  const active=records.filter(r=>!r.cancelled).slice(0,6);
  h.innerHTML=active.map((r,i)=>{
    let tag="";
    ...
    return `<div class="row"><span>${i+1}</span><span class="numberCell"><span class="historyNumber">${esc(r.value)}</span>${tag}</span><span>${esc(r.time)}</span></div>`;
  }).join("");
}

function render(){
  $("event").textContent=cfg.event;
  $("eventDate").textContent=fmtDate(cfg.date);
  $("point").textContent=cfg.point||"地点未設定";
  ...
  $("connectionState").textContent=navigator.onLine?"● 接続中":"● オフライン";
  $("connectionState").className=navigator.onLine?"connected":"offline";
  $("currentTime").textContent=now();
  $("countdown").textContent=topRemain();
  ...
  if($("rawSpeechValue")) $("rawSpeechValue").textContent=lastSpeech||"—";
  renderHistory();
  ...
  $("status").textContent=(listening?"音声受付中":"停止中")+(sendQueue.length?` ⚠未送信${sendQueue.length}件`:"");

  const b=$("manModeBtn");
  if(b){
    b.hidden=cfg.mode==="EKIDEN";
    b.innerHTML=`<span>万台</span><span>モード</span>`;
    b.style.background=manMode?"#0b57d0":"#fff";
    b.style.color=manMode?"#fff":"#c64b14";
    b.title=manMode?"万台番号モード ON":"万台番号モード OFF";
  }
  const mh=$("manModeHint"); if(mh) mh.hidden=cfg.mode==="EKIDEN";
  const rw=$("relayGapWrap"); if(rw) rw.hidden=cfg.mode!=="RELAY";
}

function refreshClock(){
  if($("currentTime")) $("currentTime").textContent=now();
  if($("countdown")) $("countdown").textContent=topRemain();
}
...
function add(value,recognized=true,rawSpeech=""){
  ...
  records.unshift(rec);if(cfg.endpoint)sendQueue.push(rec);save();render();processQueue();if(duplicate)$("status").textContent=`⚠ ${value} は重複です`;
}
...
async function deleteNumber(){
  ...
  if(!target||target==="MURI"||target==="CANCEL"){
    alert(cfg.mode==="EKIDEN"?"削除する駅伝番号を 125-3 の形式で入力してください。":"削除するナンバーを1～99999で入力してください。"); input.focus(); return;
  }
  ...
  sendQueue=sendQueue.filter(q=>q.id!==r.id); save(); render(); input.value="";
  $("status").textContent=`「${target}」${lapText}を削除しました`;
  if(cfg.endpoint&&navigator.onLine){
    try{
      await fetch(cfg.endpoint,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"DELETE",id:r.id||"",point:r.point||cfg.point||"",value:r.value||"",mode:r.mode||cfg.mode||"",event:r.event||cfg.event||"",date:r.date||cfg.date||"",staff:r.staff||cfg.staff||"",lap:r.lap||"",time:r.time||"",sheetId:r.sheetId||cfg.sheetId||""})});
      $("status").textContent=`「${target}」削除送信完了`;
    }catch{ $("status").textContent=`「${target}」はアプリから削除済・シート削除送信失敗`; }
  }
  input.focus();
}
...
function testDestination(){
  ...
  const payload={action:"PING",sheetId,event:$("sEvent").value||"接続確認",date:$("sDate").value||"",mode:$("sMode").value||"MARATHON",point:$("sPoint").value||"接続確認",staff:$("sStaff").value||"",time:now(),id:"PING_"+Date.now()};
...
$("registerBtn").addEventListener("click",registerManual);
$("numberInput").addEventListener("keydown",e=>{if(e.key==="Enter")registerManual()});
$("muriBtn").addEventListener("click",()=>add("ムリ",false,"ボタン"));
$("cancelLastBtn").addEventListener("click",cancelLast);
$("deleteNumberBtn").addEventListener("click",deleteNumber);
$("deleteNumberInput").addEventListener("keydown",e=>{if(e.key==="Enter")deleteNumber()});
...
$("testConnectionBtn").addEventListener("click",testDestination);
...
$("installBtn").addEventListener("click",async()=>{if(deferredInstall){deferredInstall.prompt();await deferredInstall.userChoice;deferredInstall=null;$("installBtn").hidden=true}});
