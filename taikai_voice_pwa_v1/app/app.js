...
function makeRecordBase(value,recognized,rawSpeech,captureTs){
  const recognizedTs=Date.now(), acceptedTs=Number(captureTs)||recognizedTs;
  return {id:recognizedTs+"_"+Math.random().toString(36).substr(2,5),seqNo:nextSeq(),value,recognized,time:timeFromTs(acceptedTs),ts:acceptedTs,captureTs:acceptedTs,recognizedTs,recognitionDelayMs:Math.max(0,recognizedTs-acceptedTs),event:cfg.event,date:cfg.date,point:cfg.point,staff:cfg.staff,mode:cfg.mode,rawSpeech,sheetId:cfg.sheetId||""};
}

function add(value,recognized=true,rawSpeech="",captureTs=0){
  if(rawSpeech)lastSpeech=rawSpeech;
  if(cfg.mode==="RELAY"&&recognized){const prev=relayLast(value);if(prev){const diff=(Date.now()-prev.ts)/1000;if(diff<cfg.relayGap*60){const rec={...makeRecordBase(value,true,rawSpeech,captureTs),invalidGap:true,invalidSeconds:Math.round(diff),lap:prev.lap};records.unshift(rec);save();render();$("status").textContent=`⚠ ${value}：${cfg.relayGap}分未満なので無効`;return;}}const lap=prev?(Number(prev.lap)||1)+1:1;const rec={...makeRecordBase(value,true,rawSpeech,captureTs),duplicate:false,lap};records.unshift(rec);if(cfg.endpoint)sendQueue.push(rec);save();render();processQueue();return;}
  const duplicate=recognized&&records.some(r=>!r.cancelled&&!r.invalidGap&&r.recognized&&r.value===value);const rec={...makeRecordBase(value,recognized,rawSpeech,captureTs),duplicate};records.unshift(rec);if(cfg.endpoint)sendQueue.push(rec);save();render();processQueue();if(duplicate)$("status").textContent=`⚠ ${value} は重複です`;
}
...
function startRecognition(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){alert("このブラウザは音声認識に対応していません。Chromeを使用してください。");return}recognition=new SR();recognition.lang="ja-JP";recognition.interimResults=false;recognition.continuous=false;recognition.onspeechstart=()=>{recognition._speechStartTs=Date.now();};recognition.onresult=e=>{const captureTs=recognition._speechStartTs||Date.now();recognition._speechStartTs=0;const t=e.results[0][0].transcript;showRecognitionRaw(t);const v=parseNumber(t);if(v==="CANCEL")cancelLast();else if(v==="MURI")add("ムリ",false,t,captureTs);else if(v)add(v,true,t,captureTs);else $("status").textContent=`認識できません：「${t}」`;};recognition.onerror=e=>{if(e.error!=="aborted")$("status").textContent="音声認識エラー："+e.error;};recognition.onend=()=>{if(listening){clearTimeout(restartTimer);restartTimer=setTimeout(()=>{try{recognition.start()}catch{}},180)}};listening=true;render();try{recognition.start()}catch{}}
