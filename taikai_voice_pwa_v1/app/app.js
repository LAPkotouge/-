...
function render(){
 $("event").textContent=cfg.event+(cfg.date?` ${cfg.date}`:"");$("point").textContent=(cfg.mode==="EKIDEN"?"駅伝":"マラソン")+" "+cfg.point;$("staff").textContent="担当："+(cfg.staff||"未設定");$("modeBadge").textContent=cfg.mode==="EKIDEN"?"駅伝":"マラソン";updateCountdown();
 $("okCount").textContent=ok;$("muriCount").textContent=muri;$("totalCount").textContent=ok+muri;const latest=records.find(r=>!r.cancelled);$("latest").textContent=latest?.value||"—";$("latestSub").textContent=latest?(latest.recognized?"番号認識":"ムリ（番号不明）"):"";
...
function openSettings(){$("sEvent").value=cfg.event;$("sDate").value=cfg.date;$("sMode").value=cfg.mode;$("sPoint").value=cfg.point;$("sStaff").value=cfg.staff;$("sTop").value=cfg.top;$("sEndpoint").value=cfg.endpoint;$("settingsPanel").hidden=false;}
function doCloseSettings(){$("settingsPanel").hidden=true;}
function doSaveSettings(){cfg={event:$("sEvent").value||"大会名未設定",date:$("sDate").value,mode:$("sMode").value,point:$("sPoint").value||"地点未設定",staff:$("sStaff").value,top:$("sTop").value,endpoint:$("sEndpoint").value};save();render();doCloseSettings();}
