...
let cfg = { event:"大会名未設定", date:"", mode:"MARATHON", point:"地点未設定", staff:"", top:"", relayGap:1, endpoint:"" };
...
function add(value,recognized=true,rawSpeech=""){
  if(cfg.mode==="RELAY"&&recognized){
    ...
    if(prev){ const diff=(Date.now()-prev.ts)/1000; if(diff<cfg.relayGap*60){ const rec={id:Date.now()+"_"+Math.random().toString(36).substr(2,5),value,recognized:true,invalidGap:true,invalidSeconds:Math.round(diff),lap:prev.lap,time:now(),ts:Date.now(),event:cfg.event,date:cfg.date,point:cfg.point,staff:cfg.staff,mode:cfg.mode,rawSpeech}; records.unshift(rec);save();render();$("status").textContent=`⚠ ${value}：${cfg.relayGap}分未満なので無効`;return; } }
    const lap=prev?(Number(prev.lap)||1)+1:1; const rec={id:Date.now()+"_"+Math.random().toString(36).substr(2,5),value,recognized:true,duplicate:false,lap,time:now(),ts:Date.now(),event:cfg.event,date:cfg.date,point:cfg.point,staff:cfg.staff,mode:cfg.mode,rawSpeech}; records.unshift(rec);if(cfg.endpoint)sendQueue.push(rec);save();render();processQueue();return;
  }
  const duplicate=recognized&&records.some(r=>!r.cancelled&&!r.invalidGap&&r.recognized&&r.value===value);
  const rec={id:Date.now()+"_"+Math.random().toString(36).substr(2,5),value,recognized,duplicate,time:now(),ts:Date.now(),event:cfg.event,date:cfg.date,point:cfg.point,staff:cfg.staff,mode:cfg.mode,rawSpeech};
...
function openSettings(){$("sEvent").value=cfg.event;$("sDate").value=cfg.date;$("sMode").value=cfg.mode;$("sPoint").value=cfg.point;$("sStaff").value=cfg.staff;$("sTop").value=cfg.top;$("sRelayGap").value=cfg.relayGap;$("sEndpoint").value=cfg.endpoint;$("settingsPanel").hidden=false;render();}
function doCloseSettings(){$("settingsPanel").hidden=true;}
function doSaveSettings(){cfg={event:$("sEvent").value||"大会名未設定",date:$("sDate").value,mode:$("sMode").value,point:$("sPoint").value||"地点未設定",staff:$("sStaff").value,top:$("sTop").value,relayGap:Math.max(1,Math.min(60,Number($("sRelayGap").value)||1)),endpoint:$("sEndpoint").value};save();render();doCloseSettings();}
